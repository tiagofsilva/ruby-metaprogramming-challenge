...
SOLUTION_FILE = './challenge/solution.rb'#ARGV[0]
...
class MetaCounterTest < Test::Unit::TestCase
  ...
  def test_integer_addition
    output = execute_program('Integer#+', <<-RB)
    10.times{ |i| i + i }
    RB
    assert_sig_and_count(output, 'Integer#+', 10)
  end
  ...
  def test_question_method
    output = execute_program('Integer#odd?', <<-RB)
    10.times{ |i| i.odd? }
    RB
    assert_sig_and_count(output, 'Integer#odd?', 10)
  end
